	...
	try {
		...
		for (let chunkPointer = 0; chunkPointer < chunkOfProjectIds.length; chunkPointer++) {
			...
			for (let projectIdpointer = 0; projectIdpointer < projectId.length; projectIdpointer++) {
				...
				let pullProjectDataDB = await db
					.collection('projects')
					.find({
						_id: id,
					})
				...
				if (pullProjectData.hasOwnProperty('tasks') && pullProjectData.tasks.length > 0) {
					...
					for (let taskCounter = 0; taskCounter < tasks.length; taskCounter++) {
						...
						if (!tasks[taskCounter]) continue

						//checks if task object has key attachment present or not
						if (
							tasks[taskCounter].hasOwnProperty('attachments') &&
							tasks[taskCounter].attachments.length > 0
						) {
							//assign attachmentPresent array to task attachments
							updateObject['$set']['tasks'][taskCounter].attachments = await validatedAttachments(
				...
				if (
					JSON.stringify(updateObject['$set']['tasks']) != allTasksBeforeValidation ||
					JSON.stringify(updateObject['$set']['attachments']) != allAttachmentsBeforeValidation
				) {
					...
					if (
						updateObject['$set'].hasOwnProperty('attachments') ||
						updateObject['$set'].hasOwnProperty('tasks')
					) {
		...
		async function validatedAttachments(attachments) {
			...
			for (let attachmentCounter = 0; attachmentCounter < attachments.length; attachmentCounter++) {
				...
				if (
					attachments[attachmentCounter].hasOwnProperty('isUploaded') &&
					!attachments[attachmentCounter].isUploaded &&
					attachments[attachmentCounter].type !== 'link'
				) {
