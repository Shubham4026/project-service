...
const projectTaskUpdateTopic =
	process.env.PROJECT_TASK_UPDATE_TOPIC && process.env.PROJECT_TASK_UPDATE_TOPIC != 'OFF'
		? process.env.PROJECT_TASK_UPDATE_TOPIC
		: 'sl-project-task-update-dev'
...
/**
 * Push project task updates to kafka.
 * @function
 * @name pushProjectTaskUpdateToKafka
 * @param {Object} message - Message data (updated project).
 */

const pushProjectTaskUpdateToKafka = function (message) {
	return new Promise(async (resolve, reject) => {
		try {
			let kafkaPushStatus = await pushMessageToKafka([
				{
					topic: projectTaskUpdateTopic,
					messages: JSON.stringify(message),
				},
			])
...
module.exports = {
	...
	pushProjectTaskUpdateToKafka: pushProjectTaskUpdateToKafka,
}
