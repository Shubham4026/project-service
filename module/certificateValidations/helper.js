...
module.exports = class certificateValidationsHelper {
	...
	static criteriaValidation(data) {
		return new Promise(async (resolve, reject) => {
			try {
				// Check if data and certificate exist
				...
				let criteria = data.certificate && data.certificate.criteria ? data.certificate.criteria : null // criteria conditions for certificate
					...
					if (condition.scope == CONSTANTS.common.PROJECT) {
						...
						if (
							...
							for (let tasksIndex = 0; tasksIndex < projectTasks.length; tasksIndex++) {
								...
								if (!projectTasks[tasksIndex]) continue
